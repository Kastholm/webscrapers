...
rss_feeds = [
    'https://via.ritzau.dk/rss/releases/latest'
]
...
for page_to_scrape in rss_urls_to_scrape:
    ...
    try:
        ...
        teaser_tag = soup.find('div', class_='text-elements__Leadtext-sc-1il5uxg-3')
        if not teaser_tag or not teaser_tag.text:
            print(f"Teaser not found in {page_to_scrape}, skipping.")
            continue
        teaser = teaser_tag.text

        # Try to get the outer div; if not found, move to the next article
        outer_div = soup.find('div', class_='release__PublicationContent-sc-6son67-0')
        if not outer_div:
            print(f"Outer div not found in {page_to_scrape}, skipping.")
            continue
        
        # Try to get the inner div; if not found, move to the next article
        inner_div = outer_div.find('div')
        if not inner_div:
            print(f"Inner div not found in outer div, skipping.")
            continue
        
        # Extract the text from the content in the inner div
        content_text = ""
        for child in inner_div.find_all(['p', 'a', '[href]', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'], recursive=False):
       ...
       #}
        ...
        Titlen skal være i almindelig sætningstilfælde (ikke camelcase), hvilket betyder, at det første ord og egennavne SKAL starte med stort bogstav.
